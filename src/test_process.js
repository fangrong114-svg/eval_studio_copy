...
if (papaParseFailed) {
  ...
  if (aggressiveData && aggressiveData.length > 0) {
    ...
    // Try splitting by tabs or multiple spaces if no URLs
    const firstLine = lines[0];
    const splitHeaders = firstLine.split(/\t| {2,}/).map(s => s.trim()).filter(Boolean);
    if (splitHeaders.length > 1) {
      headers = splitHeaders;
      data = lines.slice(1).map(line => {
        const values = line.split(/\t| {2,}/).map(s => s.trim()).filter(Boolean);
        const row = {};
        headers.forEach((h, i) => {
          row[h] = values[i] || '';
        });
...
console.log("Final data:", JSON.stringify(data[0], null, 2));
